    ...
    <div>
        <div class="card">
            <div class="flex flex-wrap items-start justify-between mb-4">
                <div>
                    <h1 class="text-3xl font-semibold mb-2">Proyectos</h1>
                    <p class="text-lg text-gray-500">Listado de proyectos, aqui se muestran todos los proyectos.</p>
                </div>
                <div>
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="Items" :lazy="true" dataKey="id"
                ...
                <Column field="tipo" header="Tipo" style="min-width: 7rem">
                    <template #body="slotProps">
                        <span><span class="font-semibold">Tipo: </span>{{ slotProps.data.tipo }}</span>
                        <br>
                        <span><span class="font-semibold">CUI: </span>{{ slotProps.data.codigo }}</span>
                        <br>
                        <span><span class="font-semibold">Meta: </span>{{ slotProps.data.meta_codigo }}</span>
                    </template>
                </Column>
                <Column field="nombre" sortable header="Nombre" style="min-width: 7rem">
                    <template #body="slotProps">
                        <span><span class="font-semibold">Nombre: </span>{{ slotProps.data.nombre }}</span>
                        <br>
                        <span><span class="font-semibold">Oficina: </span>{{ slotProps.data.oficina?.nombre }}</span>
                    </template>
                </Column>
                <Column field="programa_presupuestal" header="Programa Presupustal" style="min-width: 7rem">
                    <template #body="slotProps">
                        <span><span class="font-semibold">Prog. Pre.: </span>{{ slotProps.data.programa_presupuestal
                        }}</span>
                        <br>
                        <span><span class="font-semibold">Mod. Eje.: </span>{{ slotProps.data.modalidad_ejecucion
                        }}</span>
                        <br>
                        <span><span class="font-semibold">Fte. Fto: </span>{{ slotProps.data.fte_fto }}</span>
                    </template>
                </Column>
                <Column field="categoria" sortable header="Categoria" style="min-width: 7rem">
                    <template #body="slotProps">
                        <span>{{ slotProps.data.tipo_categoria }} - {{ slotProps.data.tipo_acceso }}</span>
                        <br>
        ...
        <Dialog v-model:visible="dialog" :style="{ width: '700px' }" header="Proyecto" :modal="true">
            <div class="flex flex-col gap-6">
                <ProyectosForm ref="elProyectosForm"
            ...
            <template #footer>
                ...
                <Button label="Guardar" icon="pi pi-check" @click="submitted = true; $refs.elProyectosForm.save()"
                    :loading="submitted" />
            </template>
        </Dialog>
...
import ProyectosForm from './ProyectosForm.vue';

const toast = useToast();

const elProyectosForm = ref(null);
const dt = ref();
const Items = ref([]);
...
function edit(row) {
    dialog.value = true;
    setTimeout(() => {
        elProyectosForm.value.setValue(row);
    }, 50);
}
...
async function deleteItem() {
    try {
        ...
        await ProyectosService.delete(item.value.id);
...
function getStatusLabel(status) {
    return status === 'EN EJECUCION' ? 'success' : status === 'PARALIZADO' ? 'info' : status === 'CULMINADO' ? 'danger' : 'secondary';
}
