    ...
    <div>
        <div class="card">
            <div class="flex flex-wrap items-start justify-between mb-4">
                <div>
                    <h1 class="text-3xl font-semibold mb-2">Oficinas</h1>
                    <p class="text-lg text-gray-500">Listado de oficinas, aqui se muestran todos las oficinas.</p>
                </div>
                <div>
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="Items" :lazy="true" dataKey="id"
                ...
                <Column field="siglas" sortable header="Siglas" style="min-width: 7rem"></Column>
                <Column field="nombre" sortable header="Nombre" style="min-width: 7rem"></Column>
                <Column field="ordinal" sortable header="Ordinal" style="min-width: 7rem"></Column>
        ...
        <Dialog v-model:visible="dialog" :style="{ width: '700px' }" header="Oficina" :modal="true">
            <div class="flex flex-col gap-6">
                <OficinasForm ref="elOficinasForm"
            ...
            <template #footer>
                ...
                <Button label="Guardar" icon="pi pi-check" @click="submitted = true; $refs.elOficinasForm.save()"
                    :loading="submitted" />
            </template>
        </Dialog>
...
import OficinasForm from './OficinasForm.vue';

const toast = useToast();

const elOficinasForm = ref(null);
const dt = ref();
const Items = ref([]);
...
function edit(row) {
    dialog.value = true;
    setTimeout(() => {
        elOficinasForm.value.setValue(row);
    }, 50);
}
...
async function deleteItem() {
    try {
        ...
        await OficinasService.delete(item.value.id);
        const nombre = item.value.nombre;
        toast.add({ severity: 'success', summary: 'Eliminado', detail: `Oficina ${nombre} eliminado`, life: 3000 });
        item.value = {};
        getData();
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Error al eliminar la Oficina', life: 3000 });
    }
}
