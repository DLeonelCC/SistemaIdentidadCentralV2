    ...
    <div>
        <div class="card">
            <div class="flex flex-wrap items-start justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-semibold mb-2">Cargos</h1>
                    <p class="text-lg text-gray-500">Listado de cargos, aqui se muestran todos los cargos.</p>
                </div>
                <div>
        ...
        <Dialog v-model:visible="dialog" :style="{ width: '700px' }" header="Cargo" :modal="true">
            <div class="flex flex-col gap-6">
                <CargosForm ref="elCargosForm"
            ...
            <template #footer>
                ...
                <Button label="Guardar" icon="pi pi-check" @click="submitted = true; $refs.elCargosForm.save()"
                    :loading="submitted" />
            </template>
        </Dialog>
...
<script setup>
    ...
    function edit(row) {
        dialog.value = true;
        setTimeout(() => {
            elCargosForm.value.setValue(row);
        }, 50);
    }
    ...
    async function deleteItem() {
        try {
            ...
            await CargosService.delete(item.value.id);
            const nombre = item.value.nombre;
            toast.add({ severity: 'success', summary: 'Eliminado', detail: `Cargo ${nombre} eliminado`, life: 3000 });
            item.value = {};
            getData();
        } catch (error) {
