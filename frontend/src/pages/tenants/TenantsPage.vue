    ...
    <div>
        <div class="card">
            <div class="flex flex-wrap items-start justify-between mb-4">
                <div>
                    <h1 class="text-3xl font-semibold mb-2">Tenants</h1>
                    <p class="text-lg text-gray-500">Listado de tenants, aqui se muestran todos los tenants.</p>
                </div>
                <div>
        ...
        <Dialog v-model:visible="dialog" :style="{ width: '700px' }" header="Tenant" :modal="true">
            <div class="flex flex-col gap-6">
                <TenantsForm ref="elTenantsForm"
            ...
            <template #footer>
                ...
                <Button label="Guardar" icon="pi pi-check" @click="submitted = true; $refs.elTenantsForm.save()"
                    :loading="submitted" />
            </template>
        </Dialog>
...
import TenantsForm from './TenantsForm.vue';

const toast = useToast();

const elTenantsForm = ref(null);
const dt = ref();
const Items = ref([]);
...
function edit(row) {
    dialog.value = true;
    setTimeout(() => {
        elTenantsForm.value.setValue(row);
    }, 50);
}
...
async function deleteItem() {
    try {
        ...
        await TenantsService.delete(item.value.id);
