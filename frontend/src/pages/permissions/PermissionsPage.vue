    ...
    <div>
        <div class="card">
            <div class="flex flex-wrap items-start justify-between mb-4">
                <div>
                    <h1 class="text-3xl font-semibold mb-2">Permisos</h1>
                    <p class="text-lg text-gray-500">Listado de permisos, aqui se muestran todos los permisos.</p>
                </div>
                <div>
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="Items" :lazy="true" dataKey="id"
                ...
                <Column field="name" sortable header="Name" style="min-width: 7rem"></Column>
                <Column field="guard_name" sortable header="Guard Name" style="min-width: 7rem"></Column>
                <Column field="label" sortable header="Label" style="min-width: 7rem"></Column>
                <Column field="module" sortable header="Modulo" style="min-width: 7rem"></Column>
        ...
        <Dialog v-model:visible="dialog" :style="{ width: '700px' }" header="Permiso" :modal="true">
            <div class="flex flex-col gap-6">
                <PermissionsForm ref="elPermissionsForm"
            ...
            <template #footer>
                ...
                <Button label="Guardar" icon="pi pi-check" @click="submitted = true; $refs.elPermissionsForm.save()"
                    :loading="submitted" />
            </template>
        </Dialog>
...
const elPermissionsForm = ref(null);
const dt = ref();
const Items = ref([]);
...
function edit(row) {
    dialog.value = true;
    setTimeout(() => {
        elPermissionsForm.value.setValue(row);
    }, 50);
}
...
async function deleteItem() {
    try {
        ...
        await PermissionsService.delete(item.value.id);
        const nombre = item.value.label;
        toast.add({ severity: 'success', summary: 'Eliminado', detail: `Permiso ${nombre} eliminado`, life: 3000 });
        item.value = {};
        getData();
    } catch (error) {
        toast.add({ severity: 'error', summary: 'Error', detail: 'Error al eliminar el Permiso', life: 3000 });
    }
}
