<template>
    <div class="p-3 mb-3 bg-white dark:bg-surface-800 rounded-md border border-surface-200 dark:border-surface-700">
        <div class="flex-col justify-start items-start gap-2 inline-flex pr-4">
            <span class="text-sm font-medium">Mode</span>
            <button type="button"
                class="inline-flex w-8 h-8 p-0 items-center justify-center surface-0 dark:surface-800 border border-surface-200 dark:border-surface-600 rounded-2xl"
    ...
    <div class="p-3 bg-white dark:bg-surface-800 rounded-md border border-surface-200 dark:border-surface-700">
        <div class="flex-col justify-start items-start gap-2 inline-flex pr-4">
...
export default {
    ...
    mounted() {
        ...
        setTimeout(() => {
            ...
            if (primaryColor) {
                ...
                this.applyTheme('primary', { name: primaryColor });
            }

            const surfaceColor = localStorage.getItem('surfaceColor');
            if (surfaceColor) {
                this.selectedSurfaceColor = surfaceColor;
                this.applyTheme('surface', this.surfaces.find(c => c.name === surfaceColor));
            }
    ...
    methods: {
        ...
        getPresetExt() {
            ...
            if (!color) return null; // Si no se encuentra, evita errores
        ...
        updateColors(type, color) {
            if (type === 'primary') {
                ...
                localStorage.setItem('primaryColor', color.name); // Guarda en localStorage
            }

            else if (type === 'surface') {
                this.selectedSurfaceColor = color.name;
                localStorage.setItem('surfaceColor', color.name); // Guarda en localStorage
            }
        ...
        onPresetChange(value) {
            ...
            localStorage.setItem('preset', value); // guardamos
